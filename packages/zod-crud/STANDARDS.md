...
hooks/commands/verbs/core 어디에도 속하지 않는 관찰 도구 박스. 각 sidecar 는 useJSONDocument 의 정상 흐름 (state · ops · selection · history) 에 **수평으로 hook 한다 (관찰만)**. 본체 데이터 동작에 영향을 주지 않는다.
...
- `sidecars/*` 진입 — recorder/replay 같은 관찰 도구 외 거부. 비즈니스 로직 (verb 합성, schema gate) 은 본체 (commands/verbs/core/hooks) 에서.
...
표준이 아닌 임시 / 휴리스틱 / 실용 어휘는 `core/` 진입 거부. `sidecars/` 도 recorder/replay 같은 관찰 도구 외엔 진입 거부.
