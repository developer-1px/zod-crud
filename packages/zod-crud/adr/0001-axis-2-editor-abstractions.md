...
**Supersedes**: SPEC.md §8 (이전 버전 — selection/focus/clipboard 비-목표 선언)
**Superseded by**: —

## Context

SPEC v1.0.0 의 §8 비-목표는 selection model · focus management · clipboard 를 라이브러리 책임 밖으로 선언했다. 근거는 "JSON tree 라이브러리는 데이터만 다루고, UI state 는 사용자 layer" 라는 분리. 그러나 이는 두 가지 사실을 잘못 합쳤다.

1. 옛 NodeId 기반 구현 (`select.ts`, `focus.ts`, `clipboard/`) 은 **구현 자체가 폐기 대상이었지** 의미가 폐기 대상은 아니었다.
2. selection · focus · clipboard buffer 는 **모두 RFC 6901 Pointer 위에서 100% JSON 직렬화로 표현 가능**하다. SPEC §0.1 의 5대 원칙을 위반하지 않는다.
...
- **Axis 2 — Editor abstractions**: Selection · Focus · Clipboard. Pointer 위에서 정의. SPEC §0.2 (6)~(10).
...
- 옛 자산의 **의미** 가 부활함 (selection set, focus filter, copy/cut/paste). 사용자는 매번 재구현하지 않음.
- 두 축 모두 표준 어휘 (RFC 6901·6902·WAI-ARIA Listbox/Tree/Grid) 위에 있으므로 30년 호환.
- selection/focus/clipboard 도 100% JSON 직렬화 → collaborative cursor·SSR·postMessage 무료.
...
- Axis 2 hook 3 개 신설로 패키지 src 파일 수 증가 (예상: 6 → 11~12).
...
## RFC 6902 batch 로 표현되는 의미

clipboard.paste 의미는 RFC 6902 op 조합으로 노출된다:

| Clipboard mode + paste target | RFC 6902 batch |
|-------------------------------|----------------|
| `copy` + 단일 source → `target` | `[{ op: "copy", from, path: target }]` |
| `copy` + 다중 sources → `target` (array) | `[{ op: "copy", from: s, path: target+i }, ...]` |
| `cut` + sources → `target` | `[{ op: "move", from: s, path: target+i }, ...]` |

paste 후 batch 가 atomic 으로 적용 (G8). schema 위반 시 전체 롤백.
...
- 신규 hook 3 개 export — 기존 사용자에게 영향 0.
- SPEC §8 정정 + §8.5 신설.
