...
**Supersedes**: ADR-0001 (어휘만 supersede, 결정 내용 보존)
**Superseded by**: —

## Context

ADR-0001 (Accepted, 2026-05-10) 이 라이브러리 헌장을 두 30년 축으로 재편했다.

- Axis 1 — Data substrate (RFC 6901 + RFC 6902 + JSON)
- Axis 2 — Editor abstractions (Selection · Focus)

같은 날 사용자 정정으로 정체성 메모리가 박혔다.

> "repo 내부에서 editor 라는 네이밍을 쓰고 있는데 editor 용은 아니고 json tree 를 다루는거라고 생각해줘"

ADR-0001 의 어휘 ("Editor abstractions", "Axis 2") 가 이 정체성과 충돌한다. 또한 후속 `/discuss` + `/grill-me` 11라운드 (2026-05-10) 에서 정체성 한 줄이 더 정밀하게 박혔고, 이 한 줄에서 자동 파생되는 헌장 어휘가 ADR-0001 의 어휘와 다르다.
...
ADR-0001 의 **결정 내용은 보존, 어휘만 supersede** 한다.

### 보존되는 결정 (ADR-0001 의 본질)
...
- **Axis 1 / Axis 2 어휘 폐기**. 두 분류축으로 나누지 않는다.
- **"Editor abstractions" 어휘 폐기**. 라이브러리는 editor 가 아니라 JSON tree 다.
- **새 헌장 어휘 — 4대 기둥 + 10 verbs + 3-layer**:
  ...
  - 3-layer (코드 위계): hooks/ (얇은 React 어댑터) → verbs/ (편집 어휘 composer, pure) → core/ (RFC 표준 substrate, pure) + sidecars/
...
- 3-layer 위계가 코드 차원에서 substrate / wrapper 분리를 강제.
- "editor" 어휘 사용자 정정과 일관됨.

### Negative

- SPEC §0 전체 재작성 필요 (P0.1).
- ADR-0001 의 어휘 ("Axis 2") 를 인용한 후속 코드 / 주석 정정 필요 (P0, P7).
- "Editor abstractions" 라는 명료한 어휘 1개를 잃고 "편집 어휘 wrapper" + 4대 기둥 + 10 verbs + 3-layer 라는 4-tuple 로 분해됨 — 한 줄 카피 비용 증가.

### Neutral

- ADR-0001 은 "Superseded by ADR-0002" 마킹. 결정 내용은 본 ADR 에 흡수되어 보존.
