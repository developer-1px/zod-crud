...
facade 의 `doc.commands.select(action, mode?)` 와 `doc.commands.selectScope(options?)` 는 `mode` 를 생략하면 문서 생성 시 설정한 selection mode 를 사용한다. 따라서 `createJSONDocument` 와 `useJSONDocument` 에서 `multiple` / `extended` 동작이 같은 기본값을 가진다.
