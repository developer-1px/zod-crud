...
- `hooks/useJSONDocument` / `createJSONDocument` — value, ops, selection, clipboard, history, commands, can, check, read/query helpers 를 한 객체로 묶는 facade. facade 의 `copy` / `cut` source 인자를 생략하면 현재 `selection.selectedSource` 를 사용하고, `duplicate` source 또는 `paste` target 인자를 생략하면 현재 `selection.primaryPointer` 를 사용한다. move/replace 같은 target-bearing command 는 명시 pointer 인자를 받는다.
...
check 는 state, selection, clipboard, history 를 바꾸지 않는 dry-run guard 이며 `can.x(...) === check.x(...).ok` 이다. source 인자를 생략한 `check.copy()` / `check.cut()` 은 현재 selection source 로 검증하고, target 인자를 생략한 `check.paste(payload)` 는 현재 primary selection target 으로 검증한다.
