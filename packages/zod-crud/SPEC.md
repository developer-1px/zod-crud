...
- `hooks/useJSONDocument` / `createJSONDocument` — value, ops, selection, history, commands, can 을 한 객체로 묶는 facade. clipboard/move/replace command 는 명시 pointer/payload 인자를 받으며, UI selection 을 command 인자로 바꾸는 일은 사용자 layer 책임.
...
(`apps/showcase`, `apps/nested-ui-lab`, 구 사이트 구현)도 SPEC §8 비-목표
영역이라 함께 제거됨. 현재 `apps/site` 는 새 문서 사이트다.
