...
- `hooks/useJSONDocument` / `createJSONDocument` — value, ops, selection, clipboard, history, commands, can, check, read/query helpers 를 한 객체로 묶는 facade. clipboard/move/replace command 는 명시 pointer/payload 인자를 받으며, UI selection 을 command 인자로 바꾸는 일은 사용자 layer 책임.
