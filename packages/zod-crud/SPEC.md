...
facade 의 `doc.check.moveCursor()` / `doc.can.moveCursor()`, `doc.check.extendCursor()` / `doc.can.extendCursor()`, `doc.check.selectScope()` / `doc.can.selectScope()` 는 같은 traversal option 으로 selection 을 바꾸지 않고 실행 가능 여부를 보고한다.
