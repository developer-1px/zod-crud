...
10. **공통 facade** — 루트 `zod-crud` 의 `createJSONDocument` 와 `zod-crud/react` 의 `useJSONDocument` 가 같은 document surface 를 제공한다. Low-level data owner 는 `createJSON`; React `useJSON` 은 그 facade 다.
...
- `verbs/*` 끼리 import 금지. 합성은 `createCommands` 또는 `createJSONDocument` facade 에서만.
...
같은 표면을 제공한다. `createJSON` / `useJSON` 는 RFC 6902 ops 와 lifecycle 만 제공하고,
...
### 5.1 `createJSON` / `useJSON` — low-level data owner
...
without React. `useJSON` is a React facade over the same headless state owner and
adds render invalidation only. Undo/redo, selection, clipboard, commands, and
...
export interface SelectionState<T> {
  ...
  selectedSource: SelectionSource | null;  // copy/cut source projection
...
React state/render lifecycle 을 얹은 같은 facade 이다. 둘 다 data, selection, clipboard, history, 10 verbs plus selection/text helpers, boolean guard predicates, explainable dry-run checks, schema introspection, `commit`, read/query helpers 를 한 객체로 묶는다.
...
React `useDebugLog` 는 같은 debug log 의 facade 이다.
...
sidecars/
  replayRecording.ts recorder.ts createDebugLog.ts debug-log.ts http.ts
```
