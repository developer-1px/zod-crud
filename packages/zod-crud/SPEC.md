...
- `hooks/useJSONDocument` / `createJSONDocument` — value, ops, selection, clipboard, history, commands, can, check, read/query helpers 를 한 객체로 묶는 facade. facade 의 `copy` / `cut` source 인자를 생략하면 현재 `selection.selectedSource` 를 사용한다. move/replace/paste 같은 target-bearing command 는 명시 pointer/payload 인자를 받는다.
