...
`compareSelectionPoints` / `orderSelectionRange` / `orderPrimarySelectionRange` 는 directional anchor/focus 를 JSON source-order, JSONPath `query` 결과 순서, 또는 명시 `points` 순서 기준의 `start`/`end` 로 환원한다. 같은 path 의 `offset` 은 numeric order 로 비교하고, ancestor point 의 `edge: "after"` 는 descendant 뒤 경계로 비교한다. `orderSelectionRanges` 는 모든 range 를 `start` 순서로 정렬하면서 원래 `selectionRanges` index 와 primary 여부를 보존한다. `selectionSpansForPointer` 는 정렬된 range 를 특정 pointer 안의 local span 으로 clip 한다. string 값이면 현재 길이로 `startOffset`/`endOffset` 을 해석하고, non-string offset domain 은 `SelectionSpanOptions.getLength` 로 앱이 길이를 주입한다. `SelectionState.orderPrimaryRange` / `orderRanges` / `spansForPointer` 는 현재 state owner 에 같은 helper 를 적용한다. delete/format/copy/paste/rendering 처럼 "선택 범위의 앞뒤" 와 pointer-local offset 이 필요한 command 는 앱마다 anchor/focus 비교를 다시 만들지 않는다.
