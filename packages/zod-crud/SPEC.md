...
hooks/      React-only. React API 의존 (useState/useMemo/useRef/MutableRefObject).
            useJSONDocument 단일 facade. useJSON, useSelection, useHistoryDispatch +
            React-context wiring helpers (buildJSONDocumentOps, jsonDocumentHistory).
   │ uses
...
10. **단일 facade** — React 진입점은 `zod-crud/react` 의 `useJSONDocument` 1개. 10 verbs 전부 + state 가 한 객체에 노출. headless 사용자는 루트 `zod-crud` 또는 `verbs/*` pure 함수 직접 import.
...
[`src/core/schema/bridge.ts`](./src/core/schema/bridge.ts) 가 zod 4 의 `toJSONSchema` / `fromJSONSchema` 를 re-export 한다. 외부 도구 (Ajv · OpenAPI · AsyncAPI · 코드젠) 가 받는 표면이 JSON Schema (draft-2020-12) 이므로 이 다리 없으면 우리 schema 가 zod 안에 갇힌다.
...
export interface SelectionState<T> {
  ...
  focus: Pointer | null;      // range 끝점. DOM Selection API 의 focus 와 동일 의미.
  isCollapsed: boolean;
...
`anchor` / `focus` 는 W3C Selection API 의 좌표 이름이다. 별도 활성 좌표 축은 없다.
collapsed selection (`anchor === focus`, `ranges.length === 1`) 이 캐럿이다.
...
- 좌표 자동 추적 (RFC 6902 op 적용 시 selection 이 추종)
...
hooks/
  ...
  useJSONDocument.ts  ─ single React facade
...
sidecars/
  recorder.ts debug-log.ts http.ts
```
