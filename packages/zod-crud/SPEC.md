...
Path arithmetic 은 모든 editor 가 공유하는 순수 path 조작. 이 5개 함수가 정본 — 사용자가 split/regex 로 직접 짜는 것을 막는다. **state·schema 모름** 이 핵심이다. JSON source-order cursor 이동은 selection subsystem 이 state 기반으로 제공하고, app-specific visible order 는 각 editor 의 spec 이 정의한다.
...
`moveSelectionCursor` / `extendSelectionCursor` 는 current selection snapshot 과 current JSON state 를 받아 JSON source-order DFS 좌표로 cursor 를 이동하거나 확장한 다음 next `SelectionSnap` 을 반환하는 pure helper 다. `resolveSelectionCursor` 는 mutation 없이 다음 target 만 계산한다. `SelectionState.moveCursor` / `extendCursor` / `resolveCursor` 는 같은 로직을 state owner 위에서 실행한다. `scope` 는 traversal root, `includeScope` 는 root 포함 여부, `wrap` 은 boundary wrap 여부다. app-specific visible order 는 이 API 위에서 사용자 코드가 별도 mapping 으로 제공한다.
...
History 의미: selection 단독 변경은 history 비대상. `createJSONDocument` facade 는 patch dispatch 시점에 selection 스냅샷을 같이 entry 에 캡처해 undo/redo 시 같이 원복하고, `useJSONDocument` 는 같은 headless document facade 를 사용한다.
