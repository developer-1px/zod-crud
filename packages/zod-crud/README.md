...
| `JSONChangeMetadata`, `JSONOps<T>` | low-level ops contract and metadata types |
...
| `JSONDocument<T>`, `JSONDocumentCommitOptions`, `JSONDocumentHistory`, `UseJSONDocumentOptions<T>`, `ClipboardPasteResult`, `ClipboardReadResult`, `ClipboardState<T>`, `ClipboardWriteOptions`, `CreateClipboardOptions<S>`, `HistoryTransactionOptions` | shared headless facade types |
