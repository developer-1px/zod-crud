...
2. **selection 자동 사용 금지** — `state.selection` 을 verb 안에서 자동 읽지 않는다. selection 이 필요하면 호출자 (`hooks/useJSONDocument`) 가 명시 인자로 넘긴다.
3. **`verbs/*` 끼리 import 금지** — 합성은 `hooks/useJSONDocument` (facade) 에서만. cut 이 copy 를 import 하지 않는다. 대신 facade 가 cut 의 결과로 copy 의 payload + remove 의 patch 를 같이 만든다.
4. **`core/*` 만 의존** — 의존 방향: verbs/ → core/. 다른 verb / hook / sidecar 의존 금지.
