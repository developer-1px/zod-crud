...
function textEditSelection(
  ...
  const ranges = edits.map((edit) => {
    ...
    return collapsedRange(point);
  });
  const primaryIndex = edits.findIndex((edit) => edit.primary);
  const next = snapFromRanges(ranges, primaryIndex >= 0 ? primaryIndex : ranges.length - 1, "multiple");
  return base.context === undefined ? next : withSelectionContext(next, base.context);
}
