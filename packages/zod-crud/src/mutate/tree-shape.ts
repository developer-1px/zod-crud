...
export function planSplit(
  ...
  try {
    const node = getNode(doc, nodeId);
    if (node.type !== "array") {
      return { ok: false, code: "invalid_target", reason: "split requires an array node.", nodeId };
    }
    if (node.parentId === null) {
    ...
    if (parent.type !== "array") {
      return { ok: false, code: "invalid_target", reason: "split target's parent must be an array (D9).", nodeId };
    }
    if (!Number.isInteger(at) || at < 0 || at > node.children.length) {
      return { ok: false, code: "invalid_target", reason: `split index out of bounds: ${at}.`, nodeId };
    }
    ...
    return { ok: true, next, changes: changesForReplacedSubtree(doc, next, parent.id), nodeId: parent.id };
  } catch (error) {
    return failure(error) as OperationFailure;
  }
}
...
export function planJoin(
  ...
  try {
    ...
    if (a.parentId !== b.parentId) {
      return { ok: false, code: "invalid_target", reason: "join requires same parent.", nodeId };
    }
    if (a.type !== b.type) {
      return { ok: false, code: "invalid_target", reason: `join requires same type; got ${a.type} and ${b.type}.`, nodeId };
    }
    if (a.type !== "array" && a.type !== "object") {
      return { ok: false, code: "invalid_target", reason: "join requires array or object nodes.", nodeId };
    }
    ...
    if (a.type === "array") {
      ...
      for (const key of Object.keys(bObj)) {
        if (key in aObj) {
          return { ok: false, code: "duplicate_key", reason: `join produces duplicate key: ${key}.`, nodeId };
        }
      }
      merged = { ...aObj, ...bObj };
    }
    ...
    if (parent.type === "array") {
      ...
      newParentValue = [...without.slice(0, insertAt), merged, ...without.slice(insertAt)];
      ...
      return { ok: false, code: "invalid_target", reason: "join's parent must be array or object.", nodeId };
    }
...
export function planIndent(doc: JsonDoc, nodeId: NodeId): IndentPlan {
  ...
  return { ok: true, targetId: parent.children[idx - 1]! };
}

export function planOutdent(doc: JsonDoc, nodeId: NodeId): IndentPlan {
  const node = doc.nodes[nodeId];
  if (node === undefined) return { ok: false, code: "invalid_target", reason: "Node not found.", nodeId };
  if (node.parentId === null) {
  ...
  return { ok: true, targetId: parent.id };
}
...
export function createTreeShape<T extends JsonValue>(deps: TreeShapeDeps<T>): TreeShapeApi {
  ...
  function commit(plan: TreeShapePlan): OperationResult {
    if (!plan.ok) return plan;
    return commitIfValid(plan.next, plan.changes, plan.nodeId);
  }

  return {
    wrap: (nodeId, key) => commit(planWrap(getDoc(), allocateNodeId, nodeId, key)),
    unwrap: (nodeId) => commit(planUnwrap(getDoc(), allocateNodeId, nodeId)),
    split: (nodeId, at) => commit(planSplit(getDoc(), allocateNodeId, nodeId, at)),
    join: (nodeId, withId) => commit(planJoin(getDoc(), allocateNodeId, nodeId, withId)),
    indent: (nodeId, moveInto) => {
      const plan = planIndent(getDoc(), nodeId);
      return plan.ok ? moveInto([nodeId], plan.targetId) : plan;
    },
    outdent: (nodeId, moveAfter) => {
      const plan = planOutdent(getDoc(), nodeId);
      return plan.ok ? moveAfter([nodeId], plan.targetId) : plan;
    },
  };
}
