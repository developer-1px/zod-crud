...
describe("doc.history performance contract", () => {
  ...
  test("large transaction merges burst edits into one undo entry", () => {
    ...
    doc.history.transaction({ label: "burst", origin: "programmatic" }, () => {
      for (let index = 1; index <= edits; index += 1) {
        expect(doc.patch({ op: "replace", path: "/value", value: index })).toEqual({ ok: true });
      }
    });
  ...
  test("root object remove history keeps __proto__ as data", () => {
    ...
    expect(doc.value).toMatchObject({ c: 3 });
    expect(Object.prototype).not.toHaveProperty("safe");
    expect(Object.prototype.hasOwnProperty.call(doc.value as object, "__proto__")).toBe(true);
    expect((doc.value as Record<string, unknown>).__proto__).toEqual({ safe: true });
  ...
  test("same-array field replace batch keeps last repeated index value", () => {
    const Schema = z.object({
  ...
  test("source-order full root record remove batches restore history", () => {
    const Schema = z.record(z.string(), z.object({ id: z.string() }));
    const initial = {
      a: { id: "a" },
      b: { id: "b" },
  ...
  test("plain structural single same-array field replace stays on local validation", () => {
    const Schema = z.object({
    ...
    const firstRow = doc.value.items[0];
    const secondRow = doc.value.items[1];

    expect(doc.patch({ op: "replace", path: "/items/1/done", value: true })).toEqual({ ok: true });

    expect(doc.value.items.map((item) => item.done)).toEqual([false, true]);
    expect(doc.value.items[0]).toBe(firstRow);
    expect(doc.value.items[1]).not.toBe(secondRow);
    expect(rootParses).toBe(0);
  });
  ...
  test("plain structural repeated same-array element replace batches stay on local validation", () => {
    ...
    expect(doc.patch([
      ...
      { op: "replace", path: "/items/0", value: { id: "a3", done: true } },
  ...
  test("descending add batch history restores in order without schema validation", () => {
    let validations = 0;
    const Schema = z.object({
      items: z.array(z.object({ id: z.string() })),
    }).superRefine(() => {
    ...
    expect(doc.patch([
      { op: "add", path: "/items/2", value: { id: "x" } },
      { op: "add", path: "/items/2", value: { id: "y" } },
      { op: "add", path: "/items/1", value: { id: "z" } },
    ])).toEqual({ ok: true });
    expect(doc.value.items.map((item) => item.id)).toEqual(["a", "z", "b", "y", "x", "c"]);
    expect(validations).toBe(validationsAfterCreate + 1);
    const validationsAfterPatch = validations;
  ...
  test("descending copy batch history restores in order without schema validation", () => {
    let validations = 0;
    const Schema = z.object({
      items: z.array(z.object({ id: z.string() })),
    }).superRefine(() => {
    ...
    expect(doc.patch([
      { op: "copy", from: "/items/0", path: "/items/4" },
      { op: "copy", from: "/items/1", path: "/items/3" },
      { op: "copy", from: "/items/0", path: "/items/3" },
    ])).toEqual({ ok: true });
    expect(doc.value.items.map((item) => item.id)).toEqual(["a", "b", "c", "a", "b", "d", "a"]);
    expect(validations).toBe(validationsAfterCreate + 1);
    const validationsAfterPatch = validations;
  ...
  test("front remove batch history restores in order without schema validation", () => {
    let validations = 0;
    const Schema = z.object({
      items: z.array(z.object({ id: z.string() })),
    }).superRefine(() => {
    ...
    expect(doc.value.items.map((item) => item.id)).toEqual(["d"]);
    expect(validations).toBe(validationsAfterCreate + 1);
    const validationsAfterPatch = validations;
  ...
  test("increasing remove batch history restores in order without schema validation", () => {
    let validations = 0;
    const Schema = z.object({
      items: z.array(z.object({ id: z.string() })),
    }).superRefine(() => {
    ...
    const initial = { items: [{ id: "a" }, { id: "b" }, { id: "c" }, { id: "d" }, { id: "e" }] };
    const doc = createJSONDocument(Schema, initial, { history: 10 });
    const validationsAfterCreate = validations;
    ...
    expect(doc.value.items.map((item) => item.id)).toEqual(["b", "d"]);
    expect(validations).toBe(validationsAfterCreate + 1);
    const validationsAfterPatch = validations;
