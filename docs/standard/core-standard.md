...
이 문서는 `zod-crud` 공개 API가 특정 구현체의 사용 설명을 넘어,
...
어떤 구현체가 zod-crud 호환이라고 주장하려면 이 문서와 적합성
suite를 통과해야 한다. README와 사이트 문서는 사용법을 설명할 수 있지만,
의미론 계약의 기준은 이 문서다.

## 2. 범위

zod-crud는 schema로 보호되는 JSON 상태 위에서 동작하는 headless 편집
foundation이다.
...
core 적합성은 root `zod-crud` entrypoint를 대상으로 한다.

React 적합성은 별도로 주장할 수 있으며, `zod-crud/react` entrypoint만
대상으로 한다.
...
zod-crud 호환 구현체는 implementation-private module 없이 적합성 suite를
통과해야 한다.
