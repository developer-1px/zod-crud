...
| Lab | 지금 승격하지 않는 이유 | 다음 gate |
| --- | --- | --- |
| `references` | `id-resolver`와 겹치는 부분이 있어 relation/backlink feature dogfood가 더 필요하다 | block docs/CMS relation mini app에서 backlinks, missing target, set reference를 검증 |
