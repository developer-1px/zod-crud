...
| stable id to Pointer | Kanban, form builder, import/review, slide/layer selection에서 반복 | recipe 후보 | id policy 차이를 보존한 small lab이 가능한지 확인 |
...
따라서 현재 판정은 recipe 후보가 맞다.

```txt
stable id resolver
|-- core 아님: id semantics를 모름
|-- official 아직 아님: 정책 차이가 큼
|-- lab 가능: descriptor로 id field와 collection scope를 받는 실험
