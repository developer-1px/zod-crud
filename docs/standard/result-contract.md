...
모든 실행 result는 `ok` discriminant를 가져야 한다.
...
| undo/redo | next `value`, `applied` 또는 복원 metadata |
...
Document execution method의 strict 실패는 `JSONCrudError`를 throw할 수 있다.
Non-strict 실패는 result object를 반환해야 한다.

`JSONCrudError`도 stable `code`와 optional diagnostic을 보존해야 한다. 앱은
