...
::source{path="apps/site/src/examples/RejectedDrift.tsx" title="RejectedDrift.tsx"}

`z.number().max(100)` 위반 시 `JsonResult.code = "schema_violation"`. `strict: false` 라 throw 하지 않고 `onError` 와 반환값으로 통보합니다.

## 첫 patch — React 없이

::source{path="apps/site/src/examples/snippet-getting-started.ts" title="snippet"}
