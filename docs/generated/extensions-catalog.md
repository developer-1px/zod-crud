...
| `@zod-crud/cycle` | lab-only | 10 | toggle a boolean or advance an enum/value field (enum options come from the schema) | rendered toggle controls or keyboard policy | Lab cycle extension for `zod-crud` documents. |
...
| `@zod-crud/truncate` | lab-only | 9 | cap a string field to a max length with optional ellipsis and word boundary | display-time CSS truncation or grapheme/locale-aware length | Lab truncate extension for `zod-crud` documents. |
