...
| `@zod-crud/grid-range` | lab-only | 27 | paste or fill rectangular grid ranges backed by sparse JSON records | DOM grid selection, coordinate naming policy, formulas, or TSV/CSV parsing | Lab grid-range editing extension for sparse-record-backed `zod-crud` documents. |
