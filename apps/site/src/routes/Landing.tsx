  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-10 p-8">
      ...
      <section aria-labelledby="quick" className="flex flex-col gap-3">
        <h2 id="quick" className="text-sm font-semibold uppercase tracking-wide text-stone-500">
          Where to next
        </h2>
          ...
          {entries.map((a) => (
            <li key={a.id}>
              <Link
                ...
                className="block border border-stone-200 p-4 hover:border-stone-900"
              >
                <div className="text-xs font-medium uppercase tracking-wider text-stone-400">
                  {paletteCategory(a)}
                </div>
                <div className="mt-1 text-sm font-medium text-stone-900">{a.label}</div>
                {a.sub && <div className="mt-1 text-xs text-stone-500">{a.sub}</div>}
              </Link>
            </li>
          ))}
        </ul>
      ...
      <section aria-labelledby="invariants" className="flex flex-col gap-3">
        <h2 id="invariants" className="text-sm font-semibold uppercase tracking-wide text-stone-500">
          Core invariants
        </h2>
        <ul className="m-0 flex list-none flex-col gap-2 p-0 text-sm text-stone-700">
