...
(`keymap`, `clipboard`, `commands`) 를 합쳐 5축(키보드 · cursor ·
multi-select · clipboard · error) 을 모두 구현한 Workflowy/Roam 풍
outliner.
...
구조 명령 (Tab/Shift+Tab/Mod+ArrowUp/Down) 과 history (Mod+Z/Mod+Shift+Z/Mod+Y) 는
**두 모드 모두**에서 동작 (mode-independent).
...
(`enter-edit`, `exit-edit`) 은 `useDispatch`/`Outliner` local state 에서
처리한다.
