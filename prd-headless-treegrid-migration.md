# PRD — Showcase JsonTreeGrid를 @p/headless로 재배선
...
zod-crud showcase의 JSON treegrid playground는 `@p/headless`를 의존성으로 선언만 하고 실제로는 import하지 않는다. JsonTreeGrid는 자체 `onKeyDown` if-분기로 키 매칭을 직접 작성했고, 그 결과:
...
- `eventSelectionMode`/`SelectionMode` 자체 타입으로 multi-select 분기를 재발명 — `@p/headless`의 `multiSelect` axis가 이미 제공
...
JsonTreeGrid를 `@p/headless`의 `useTreeGridPattern`으로 재배선한다. 자체 onKeyDown을 제거하고 chord registry · roving tabindex · ARIA emit · multiSelect axis를 모두 패턴에 위임한다. headless 어휘에 부족한 부분(셀 인라인 값 편집, 클립보드/히스토리 명령)은 ds 라이브러리 측에 1급 어휘로 추가하여 다른 소비자도 재사용 가능하게 한다.
...
- nested-ui-lab 앱 — 현재 `@p/headless` 미의존이며 이번 마이그레이션 범위 밖
...
- `apps/showcase/package.json`의 `@p/headless`는 `file:../../../ds/packages/headless` 링크. ds 빌드 산출물 갱신 시 zod-crud 측 `npm install`로 흡수
