...
const adapter: CompatiblePasteAdapter = {
  adapt({ payload }) {
...
describe("@zod-crud/paste-compatible", () => {
  ...
  test("adapts array payloads to spread paste with ID remapping", () => {
    ...
    expect(pasteCompatible(doc, adapter, {
      payload: [
  ...
  test("reports adapter exceptions as structured errors", () => {
    const doc = createBoard();
    const throwingAdapter: CompatiblePasteAdapter = {
      adapt() {
        throw new Error("adapter boom");
      },
    };

    expect(canPasteCompatible(doc, throwingAdapter, {
  ...
  test("preserves core schema rejection when adapted payload still does not fit", () => {
    const doc = createBoard();
    const noRekeyAdapter: CompatiblePasteAdapter = {
      adapt({ payload }) {
        if (!isExternalCard(payload)) {
    ...
    expect(canPasteCompatible(doc, noRekeyAdapter, {
    ...
    expect(pasteCompatible(doc, noRekeyAdapter, {
      payload: { type: "external.card", id: "a", name: "Duplicate" },
      target: "/cards/-",
    })).toMatchObject({
      ok: false,
