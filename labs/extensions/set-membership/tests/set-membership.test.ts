...
import { createSetMembership, plan, type SetMembershipResult } from "../src/index.js";
...
function expectOk(result: SetMembershipResult): Extract<SetMembershipResult, { ok: true }> {
  if (!result.ok) throw new Error(`expected ok, got ${result.code}: ${result.reason}`);
  return result;
}

describe("@zod-crud/set-membership", () => {
