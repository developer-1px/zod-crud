...
describe("@zod-crud/bulk-edit", () => {
  test("replaces all JSONPath matches through public document patching", () => {
    ...
    expect(bulk.canReplaceQuery("$.tasks[*].done", true)).toMatchObject({
      ok: true,
      count: 5,
    });
    expect(bulk.replaceQuery("$.tasks[*].done", true)).toMatchObject({
      ok: true,
      count: 5,
    });
  ...
  test("maps replacement values from the current query matches", () => {
    ...
    expect(replaceQuery(doc, "$.tasks[*].title", ({ value }) => String(value).trim())).toMatchObject({
      ok: true,
      count: 5,
    });
  ...
  test("deletes array matches from the back to avoid pointer drift", () => {
    ...
    expect(deleteQuery(doc, "$.tasks[1:4]")).toMatchObject({
      ok: true,
      count: 3,
  ...
  test("deduplicates repeated JSONPath pointer matches before patching", () => {
    ...
    expect(deleteQuery(doc, "$.tasks[1,1]")).toMatchObject({
      ok: true,
      count: 1,
      pointers: ["/tasks/1"],
    });
  ...
  test("rejects schema-invalid replacements without mutating", () => {
    ...
    expect(canReplaceQuery(doc, "$.tasks[*].title", 123)).toMatchObject({
      ok: false,
      code: "patch_rejected",
      capability: {
        ok: false,
    ...
    expect(replaceQuery(doc, "$.tasks[*].title", 123)).toMatchObject({
      ok: false,
      code: "patch_rejected",
    });
  ...
  test("reports invalid and empty queries as extension errors", () => {
    ...
    expect(bulk.canDeleteQuery("$[")).toMatchObject({
      ok: false,
      code: "invalid_query",
    });
    expect(bulk.deleteQuery("$.missing[*]")).toMatchObject({
      ok: false,
      code: "empty_match",
    });
  ...
  test("exports standalone delete preflight", () => {
    ...
    expect(canDeleteQuery(doc, "$.tasks[?(@.archived==true)]")).toMatchObject({
      ok: true,
      count: 3,
    });
  });
