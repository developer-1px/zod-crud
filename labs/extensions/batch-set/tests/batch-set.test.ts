...
import { canBatchSet, createBatchSet, type BatchSetResult } from "../src/index.js";
...
function expectOk(result: BatchSetResult): Extract<BatchSetResult, { ok: true }> {
  if (!result.ok) throw new Error(`expected ok, got ${result.code}: ${result.reason}`);
  return result;
}

describe("@zod-crud/batch-set", () => {
  test("sets one field across selected items to a constant", () => {
    const doc = createDoc();
    const b = createBatchSet(doc);

    const result = expectOk(b.batchSet(["/rows/0", "/rows/1"], { value: "done" }, { field: "/status" }));
  ...
  test("computes the value per target", () => {
    ...
    expectOk(
      b.batchSet<number>(["/rows/0", "/rows/2"], { compute: (current) => (current as number) * 10 }, { field: "/n" }),
    );
  ...
  test("only counts writes that change a value", () => {
    ...
    const result = expectOk(b.batchSet(["/rows/0", "/rows/1", "/rows/2"], { value: "done" }, { field: "/status" }));
  ...
  test("replaces whole items when no field is given", () => {
    ...
    expectOk(b.batchSet(["/rows/1"], { value: { id: "b", status: "done", n: 9 } }));
  ...
  test("a schema-violating value is rejected by canPatch, nothing applied", () => {
    ...
    const result = b.batchSet(["/rows/0", "/rows/1"], { value: "nope" }, { field: "/status" });
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.code).toBe("patch_rejected");
    expect(doc.value.rows.map((r) => r.status)).toEqual(["todo", "todo", "done"]);
  });

  test("canBatchSet does not mutate", () => {
    const doc = createDoc();
    const result = expectOk(canBatchSet(doc, ["/rows/0"], { value: "done" }, { field: "/status" }));
  ...
  test("a missing target is rejected", () => {
    const doc = createDoc();
    const result = canBatchSet(doc, ["/rows/0", "/rows/9"], { value: "done" }, { field: "/status" });
    expect(result.ok).toBe(false);
  ...
  test("empty targets are rejected", () => {
    const doc = createDoc();
    const result = canBatchSet(doc, [], { value: "done" }, { field: "/status" });
    expect(result.ok).toBe(false);
  ...
  test("a compute that throws is reported", () => {
    const doc = createDoc();
    const result = canBatchSet(doc, ["/rows/0"], {
      compute: () => {
