...
import { canStep, createNumberStep, type NumberStepResult } from "../src/index.js";
...
function expectOk(result: NumberStepResult): Extract<NumberStepResult, { ok: true }> {
  if (!result.ok) throw new Error(`expected ok, got ${result.code}: ${result.reason}`);
  return result;
}

describe("@zod-crud/number-step", () => {
  test("increments by 1 by default", () => {
    const doc = createDoc();
    const n = createNumberStep(doc);
