...
describe("@zod-crud/document-outline", () => {
  test("builds a pointer-first tree and preorder flat list", () => {
    const outline = createDocumentOutline(createBoard());
  ...
  test("limits expansion depth while preserving child counts", () => {
    const outline = createDocumentOutline(createBoard(), "", { maxDepth: 1 });
    if (!outline.ok) throw new Error(outline.reason);
  ...
  test("can start from a nested pointer", () => {
    const outline = createDocumentOutline(createBoard(), "/lists/0/cards", { maxDepth: 1 });
    if (!outline.ok) throw new Error(outline.reason);
  ...
  test("optionally includes isolated JSON value snapshots", () => {
    ...
    const outline = createDocumentOutline(doc, "/lists/0/cards/0", { includeValues: true });
    if (!outline.ok) throw new Error(outline.reason);
    ...
    expect(createDocumentOutline(doc, "/lists/0/cards/0", { includeValues: true })).toMatchObject({
      ok: true,
      root: {
  ...
  test("omits values by default for lightweight outline reads", () => {
    const outline = createDocumentOutline(createBoard(), "/lists/0/cards/0");
    if (!outline.ok) throw new Error(outline.reason);
  ...
  test("reports pointer read errors without throwing", () => {
    expect(createDocumentOutline(createBoard(), "/missing")).toMatchObject({
      ok: false,
      code: "path_not_found",
      pointer: "/missing",
    });
    expect(createDocumentOutline(createBoard(), "not/a/pointer")).toMatchObject({
      ok: false,
