...
export type DocumentOutlineErrorCode =
  | "invalid_pointer"
  | "path_not_found";

export interface DocumentOutlineError {
  ok: false;
  code: DocumentOutlineErrorCode;
  reason?: string;
  pointer: Pointer;
}
...
export interface DocumentOutlineNode {
  ...
  expandable: boolean;
  value?: unknown;
  children?: ReadonlyArray<DocumentOutlineNode>;
}

export type DocumentOutlineResult =
  | {
      ok: true;
      root: DocumentOutlineNode;
      nodes: ReadonlyArray<DocumentOutlineNode>;
    }
...
type BuildNodeResult =
  | { ok: true; node: DocumentOutlineNode }
  | DocumentOutlineError;

export function createDocumentOutline<T>(
  doc: JSONDocument<T>,
  rootPointer: Pointer = "",
...
function copyNodes(nodes: ReadonlyArray<DocumentOutlineNode>): ReadonlyArray<DocumentOutlineNode> {
  return nodes.map(copyNode);
}

function copyNode(node: DocumentOutlineNode): DocumentOutlineNode {
  const copied: DocumentOutlineNode = {
    key: node.key,
    path: node.path,
    depth: node.depth,
...
function normalizeOptions(options: DocumentOutlineOptions): NormalizedOptions {
  return {
...
function outlineError(
  ...
  const error: DocumentOutlineError = { ok: false, code, pointer };
