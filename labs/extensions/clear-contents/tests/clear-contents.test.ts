...
import { canClearContents, createClearContents, type ClearContentsResult } from "../src/index.js";
...
function expectOk(result: ClearContentsResult): Extract<ClearContentsResult, { ok: true }> {
  if (!result.ok) throw new Error(`expected ok, got ${result.code}: ${result.reason}`);
  return result;
}

describe("@interactive-os/json-document-clear-contents", () => {
  test("clears scalar fields to their type-empty value", () => {
    const doc = createDoc();
    const clearer = createClearContents(doc);

    expectOk(clearer.clearContents(["/title", "/count", "/done"]));
  ...
  test("prefers the schema default over a type-empty value", () => {
    ...
    expectOk(clearer.clearContents(["/qty"]));
  ...
  test("clears arrays to [] and records to {}", () => {
    ...
    expectOk(clearer.clearContents(["/tags", "/bag"]));
  ...
  test("a host emptyFor supplies values for non-derivable kinds", () => {
    ...
    expectOk(clearer.clearContents(["/status"], { emptyFor: () => "b" }));
  ...
  test("emptyFor is authoritative even for derivable kinds", () => {
    ...
    expectOk(clearer.clearContents(["/title"], { emptyFor: () => "host-default" }));
  ...
  test("a clear that violates the schema is rejected by canPatch", () => {
    ...
    const result = clearer.clearContents(["/name"]);
    expect(result.ok).toBe(false);
  ...
  test("clearing an already-empty field is a no-op", () => {
    ...
    const result = expectOk(clearer.clearContents(["/title"]));
    expect(result.changed).toBe(false);
    expect(result.operations).toEqual([]);
  });

  test("canClearContents does not mutate the document", () => {
    const doc = createDoc();
    const result = expectOk(canClearContents(doc, ["/title", "/count"]));
  ...
  test("an empty target is rejected", () => {
    const doc = createDoc();
    const result = canClearContents(doc, []);
    expect(result.ok).toBe(false);
