...
export interface DropIntentInput {
  source: DropIntentSource;
  target: DropIntentTarget;
}

export type DropIntentErrorCode =
  | "invalid_target"
  | "unsupported_target";

export interface DropIntentError {
  ok: false;
  code: DropIntentErrorCode;
  reason: string;
  pointer?: Pointer;
}

export interface DropIntentPlan {
  ok: true;
  kind: DropIntentSource["kind"];
  target: Pointer | JSONDocumentPasteTarget;
  capability: JSONCapabilityResult;
}

export type DropIntentPlanResult =
  | DropIntentPlan
  | DropIntentError;

export interface DropIntentPerformOk {
  ok: true;
  kind: DropIntentSource["kind"];
  target: Pointer | JSONDocumentPasteTarget;
  result: unknown;
...
export interface DropIntentPerformError {
  ok: false;
  code: "disabled" | "execution_failed" | DropIntentErrorCode;
  reason: string;
  pointer?: Pointer;
  capability?: JSONCapabilityResult;
  result?: unknown;
...
export type DropIntentPerformResult =
  | DropIntentPerformOk
  | DropIntentPerformError;

export interface DropIntent<TDocument> {
  canDrop(input: DropIntentInput): DropIntentPlanResult;
  perform(input: DropIntentInput): DropIntentPerformResult;
}

export function createDropIntent<TDocument>(
  doc: JSONDocument<TDocument>,
): DropIntent<TDocument> {
  return {
    canDrop(input) {
...
export function canDrop<TDocument>(
  doc: JSONDocument<TDocument>,
  input: DropIntentInput,
): DropIntentPlanResult {
...
export function performDrop<TDocument>(
  doc: JSONDocument<TDocument>,
  input: DropIntentInput,
): DropIntentPerformResult {
  const plan = canDrop(doc, input);
  if (!plan.ok) return plan;
  if (!plan.capability.ok) {
    const error: DropIntentPerformError = {
      ok: false,
      code: "disabled",
...
function moveTarget(source: Pointer, target: DropIntentTarget): { ok: true; pointer: Pointer } | DropIntentError {
...
function pasteTarget(target: DropIntentTarget): { ok: true; target: JSONDocumentPasteTarget } | DropIntentError {
...
): { ok: true; pointer: Pointer } | DropIntentError {
...
function pasteOptions(source: Extract<DropIntentSource, { kind: "payload" }>): JSONDocumentPasteOptions {
