...
import { canPasteGrid, createGridPaste, type GridPasteResult } from "../src/index.js";
...
function expectOk(result: GridPasteResult): Extract<GridPasteResult, { ok: true }> {
  if (!result.ok) throw new Error(`expected ok, got ${result.code}: ${result.reason}`);
  return result;
}

describe("@zod-crud/grid-paste", () => {
