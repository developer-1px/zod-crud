...
Lab grid-paste extension for `zod-crud` documents.
...
```ts
import { createGridPaste } from "@zod-crud/grid-paste";

const grid = createGridPaste(doc);
...
distinct from `paste-compatible`:

- **`paste-compatible` adapts payload _shape_** — one payload reshaped/ID-remapped
  to fit a single target slot.
- **`grid-paste` maps a _rectangle_** — a 2D matrix distributed across a
