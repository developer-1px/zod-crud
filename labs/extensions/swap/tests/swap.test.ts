...
import { canSwap, createSwap, type SwapResult } from "../src/index.js";
...
function expectOk(result: SwapResult): Extract<SwapResult, { ok: true }> {
  if (!result.ok) throw new Error(`expected ok, got ${result.code}: ${result.reason}`);
  return result;
}

describe("@zod-crud/swap", () => {
  test("swaps two items", () => {
    const doc = createDoc();
    const s = createSwap(doc);

    expectOk(s.swap("/items/0", "/items/2"));
  ...
  test("swapping adjacent items", () => {
    ...
    expectOk(s.swap("/items/1", "/items/2"));
  ...
  test("swapping an item with itself is a no-op", () => {
    ...
    const result = expectOk(s.swap("/items/1", "/items/1"));
  ...
  test("swapping equal values is a no-op", () => {
    ...
    const result = expectOk(s.swap("/items/0", "/items/1"));
    expect(result.changed).toBe(false);
  });

  test("canSwap does not mutate", () => {
    const doc = createDoc();
    const result = expectOk(canSwap(doc, "/items/0", "/items/3"));
  ...
  test("rejects pointers in different parents", () => {
    const doc = createDoc();
    const result = canSwap(doc, "/items/0", "/other/0");
  ...
  test("rejects a non array-item pointer", () => {
    const doc = createDoc();
    const result = canSwap(doc, "/items", "/items/0");
    expect(result.ok).toBe(false);
  ...
  test("rejects an out-of-range index", () => {
    const doc = createDoc();
    const result = canSwap(doc, "/items/0", "/items/9");
    expect(result.ok).toBe(false);
