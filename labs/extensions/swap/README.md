...
- Expose `canSwap` beside `swap`.

## Non-goals

- Cross-array swaps, or moving an item to an arbitrary index (use
  `@zod-crud/move-selection` / `@zod-crud/collection`).
...
does not exchange them. swap computes the exchanged array directly and applies it
