...
import { canTransform, createTextTransform, type TextTransformResult } from "../src/index.js";
...
function expectOk(result: TextTransformResult): Extract<TextTransformResult, { ok: true }> {
  if (!result.ok) throw new Error(`expected ok, got ${result.code}: ${result.reason}`);
  return result;
}

describe("@zod-crud/text-transform", () => {
  test("upper / lower", () => {
    const doc = createDoc("Hello");
    const t = createTextTransform(doc);
