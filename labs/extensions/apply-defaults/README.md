...
a value"), the additive complement to `fill-blanks` (which fills existing empty
slots). Nested/deep defaulting stays host-owned.

Like `convert-type` and `clear-contents`, the schema is the source of truth: apply-defaults
