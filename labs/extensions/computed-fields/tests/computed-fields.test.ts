...
const fields: Array<ComputedFieldDefinition<DocumentValue>> = [
  {
    key: "slug",
    path: "/slug",
...
describe("@zod-crud/computed-fields", () => {
  test("plans computed field replacements without mutating", () => {
    const doc = createDoc();

    expect(planComputedFields(doc, fields)).toMatchObject({
      ok: true,
      changed: true,
      fields: [
        {
  ...
  test("syncs computed fields through patch", () => {
    const doc = createDoc();
    const computed = createComputedFields(doc, fields);
  ...
  test("returns unchanged when computed values are already current", () => {
    ...
    syncComputedFields(doc, fields);

    expect(planComputedFields(doc, fields)).toEqual({
      ok: true,
      changed: false,
      fields: [
        {
  ...
  test("returns isolated computed plans", () => {
    ...
    const next = planComputedFields(doc, fields);
