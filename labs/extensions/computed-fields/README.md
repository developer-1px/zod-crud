...
Lab computed field extension for `zod-crud` documents.
...
```ts
import { createComputedFields } from "@zod-crud/computed-fields";

const computed = createComputedFields(doc, [
  {
    path: "/stats/total",
...
- Let host code define computed field functions.
...
- No formula language, dependency graph, spreadsheet engine, cycle detection, or
  scheduler.
...
The public facade is enough for simple computed fields. Host code owns formulas,
