...
const descriptor: BlockTypeConversionDescriptor = {
  targetTypes: ["paragraph", "heading", "todo", "broken", "throw"],
  readType(value) {
...
describe("@zod-crud/convert-block-type", () => {
  ...
  test("preserves compatible fields through host factory rules", () => {
    ...
    expect(convertBlockType(doc, descriptor, { pointer: "/blocks/1", to: "todo" })).toMatchObject({
      ok: true,
      operation: {
        value: {
          kind: "todo",
  ...
  test("reports invalid target and unsupported source type", () => {
    const doc = createPage();

    expect(canConvertBlockType(doc, descriptor, { pointer: "/blocks/99", to: "heading" })).toMatchObject({
      ok: false,
      code: "invalid_target",
      pointer: "/blocks/99",
    });
    expect(canConvertBlockType(doc, descriptor, { pointer: "/title", to: "heading" })).toMatchObject({
      ok: false,
  ...
  test("reports invalid target type and host conversion denial", () => {
    const doc = createPage();

    expect(canConvertBlockType(doc, descriptor, { pointer: "/blocks/0", to: "callout" })).toMatchObject({
      ok: false,
      code: "invalid_target_type",
      from: "paragraph",
      to: "callout",
    });
    expect(canConvertBlockType(doc, descriptor, { pointer: "/blocks/0", to: "paragraph" })).toMatchObject({
      ok: false,
      code: "invalid_target_type",
    ...
    expect(canConvertBlockType(doc, descriptor, { pointer: "/blocks/0", to: "heading" })).toMatchObject({
      ok: false,
      code: "invalid_target_type",
  ...
  test("reports factory failure without mutating", () => {
    ...
    expect(convertBlockType(doc, descriptor, { pointer: "/blocks/0", to: "throw" })).toMatchObject({
      ok: false,
      code: "factory_failed",
      reason: "factory boom",
      from: "paragraph",
  ...
  test("reports schema rejection from core canPatch", () => {
    ...
    expect(convertBlockType(doc, descriptor, { pointer: "/blocks/0", to: "broken" })).toMatchObject({
      ok: false,
      code: "patch_rejected",
      from: "paragraph",
      to: "broken",
