...
import { canMoveSelection, createMoveSelection, type MoveSelectionResult } from "../src/index.js";
...
function expectOk(result: MoveSelectionResult): Extract<MoveSelectionResult, { ok: true }> {
  if (!result.ok) throw new Error(`expected ok, got ${result.code}: ${result.reason}`);
  return result;
}

describe("@zod-crud/move-selection", () => {
  test("moves a block after a later sibling, preserving internal order", () => {
    const doc = createDoc();
    const mover = createMoveSelection(doc);

    const result = expectOk(mover.moveSelection(["/rows/1", "/rows/2"], { after: "/rows/3" }));
  ...
  test("moves a single item up (before an earlier sibling)", () => {
    ...
    expectOk(mover.moveSelection(["/rows/3"], { before: "/rows/1" }));
    expect(ids(doc)).toEqual(["a", "d", "b", "c", "e"]);
  });
  ...
  test("moves a leading block to the end (after the last sibling)", () => {
    ...
    const result = expectOk(mover.moveSelection(["/rows/0", "/rows/1"], { after: "/rows/4" }));
  ...
  test("moving a block to its current position is a no-op", () => {
    ...
    const result = expectOk(mover.moveSelection(["/rows/1", "/rows/2"], { after: "/rows/0" }));
    expect(result.changed).toBe(false);
    expect(result.operations).toEqual([]);
    expect(ids(doc)).toEqual(["a", "b", "c", "d", "e"]);
  });

  test("canMoveSelection does not mutate the document", () => {
    const doc = createDoc();
    const result = expectOk(canMoveSelection(doc, ["/rows/0"], { after: "/rows/3" }));
  ...
  test("a target inside the moved selection is rejected", () => {
    const doc = createDoc();
    const result = canMoveSelection(doc, ["/rows/1", "/rows/2"], { before: "/rows/2" });
  ...
  test("a non-contiguous source is rejected", () => {
    const doc = createDoc();
    const result = canMoveSelection(doc, ["/rows/0", "/rows/2"], { after: "/rows/3" });
    expect(result.ok).toBe(false);
  ...
  test("a source spanning two parents is rejected", () => {
    const doc = createDoc();
    const result = canMoveSelection(doc, ["/rows/0", "/other/0"], { after: "/rows/3" });
    expect(result.ok).toBe(false);
  ...
  test("a target in a different parent is rejected", () => {
    const doc = createDoc();
    const result = canMoveSelection(doc, ["/rows/0", "/rows/1"], { after: "/other/0" });
    expect(result.ok).toBe(false);
  ...
  test("a source that is not an array item is rejected", () => {
    const doc = createDoc();
    const result = canMoveSelection(doc, ["/rows"], { after: "/rows/3" });
    expect(result.ok).toBe(false);
  ...
  test("an empty source is rejected", () => {
    const doc = createDoc();
    const result = canMoveSelection(doc, [], { after: "/rows/3" });
    expect(result.ok).toBe(false);
  ...
  test("source given out of order is normalized to a contiguous block", () => {
    ...
    expectOk(mover.moveSelection(["/rows/2", "/rows/1"], { after: "/rows/3" }));
    expect(ids(doc)).toEqual(["a", "d", "b", "c", "e"]);
  });
});
